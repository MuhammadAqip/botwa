...
conn.sendMessage(id, donate.donate(id, BotName) ,MessageType.text);
}
else if (text == '.donasi'){
conn.sendMessage(id, donate.donate(id, BotName) ,MessageType.text);
}
...
conn.sendMessage(id, info.info(id, BotName) ,MessageType.text);
}
else if (text == '.ptl'){
   ...
   if (text.includes(".ptl cewek"))
   {
    ...
    axios.get(url)
      .then((result) => {
        ...
        .then(
            (response) => {
	var buf = Buffer.from(response, 'base64'); // Nih Bro
              conn.sendMessage(
            id,
